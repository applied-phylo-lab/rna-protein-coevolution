...
Model_OUOU_independent_evolution <- function(parm, Data)
{
  ...
  HYP <-  sum(dlnorm(h,meanlog = log(log(2)/(173/4)),sdlog = 1.5,log=T)) + sum(dlnorm(sigma,meanlog = log(0.25),sdlog = 1.5,log=T))
...
Model_OUOU_correlated_evolution <- function(parm, Data)
{
  ...
  HYP <-  sum(dlnorm(h,meanlog = log(log(2)/(173/4)),sdlog = 1.5,log=T)) + sum(dlnorm(sigma,meanlog = log(0.25),sdlog = 1.5,log=T)) + dnorm(b,mean = 0,sd = 1,log=T)
  PR <- sum(dnorm(parm[pos.psi[1]:length(parm)],mean = 0,sd = 1,log=T))
  LP <- LL + PR + HYP
  Modelout <- list(LP=LP,Dev=-2*LL, Monitor=c(LP,LL,PR,HYP), yhat=1, parm=parm,Parameter.LL=gene.ll)
  return(Modelout)
...
Model_OUOU_correlated_evolution_mRNA <- function(parm, Data)
{
  ...
  HYP <- sum(dlnorm(h,meanlog = log(log(2)/(173/4)),sdlog = 1.5,log=T)) + sum(dlnorm(sigma,meanlog = log(0.25),sdlog = 1.5,log=T)) + sum(dnorm(c(c.val,b),mean = 0,sd = 1,log=T))
  PR <- sum(dnorm(parm[pos.psi[1]:length(parm)],mean = 0,sd = 1,log=T))
  LP <- LL + PR + HYP
  Modelout <- list(LP=LP,Dev=-2*LL, Monitor=c(LP,LL,PR,HYP), yhat=1, parm=parm,Parameter.LL=gene.ll)
  return(Modelout)
...
cmd <- paste("Rscript --vanilla R_scripts/runMCMC_OUOU_real_data.R",cmd,sep=" ")
...
if (independent.evo)
{
  ...
  Model_OUOU <- Model_OUOU_correlated_evolution_mRNA
} else {
...
alpha.start <- log(2)/(ips::tipHeights(tree)[1]/4)
...
{
  start.values <- c(log(alpha.start),log(alpha.start),1,1,log(0.25),log(0.25)) # will propose H, Sigma on log-scale
