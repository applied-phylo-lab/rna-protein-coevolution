...
LaplacesDemon_local <- function(Model, Data, Initial.Values, Covar=NULL,
                                                         ...
                                                         DB.MCSE=FALSE, DB.Model=TRUE), LogFile="", ...)
{
  ...
  if(Algorithm == "Adaptive Directional Metropolis-within-Gibbs") {
    ...
    mcmc.out <- .mcmcrwm_local(Model, Data, Iterations, Status, Thinning,
                         Specs, Acceptance, Dev, DiagCovar, LIV, Mon, Mo0, ScaleF,
                         thinned, tuning, VarCov, Debug, LogFile)}
    ...
    mcmc.out <- .mcmcram_local(Model, Data, Iterations, Status, Thinning,
                         Specs, Acceptance, Dev, DiagCovar, LIV, Mon, Mo0, ScaleF,
                         thinned, VarCov, Debug, LogFile)}
...
.mcmcram_local <- function(Model, Data, Iterations, Status, Thinning, Specs,
  ...
  if(B == 0) {
    ...
    for (iter in 1:Iterations) {
      ...
      if(inherits(Mo1, "try-error")) {
        ...
        if(!is.finite(log.alpha)) 
        {
          log.alpha <- -Inf
        } else if (log.alpha > 0)
        {
          log.alpha <- 0 # setting minimum to 1. 
        }
        if(log.u < log.alpha) {
          Mo0 <- Mo1
      ...
      ### Adaptation
      S <- adapt_S(S,U,exp(log.alpha),iter,target=alpha.star,gamma=gamma)
      VarCov <- S %*% t(S)
    ...
    for (iter in 1:Iterations) {
      ...
      for (b in 1:B) {
        ...
        prop <- Mo0[["parm"]]
        prop[Block[[b]]] <- prop[Block[[b]]] + c(S[[b]] %*% U)
        #prop[Block[[b]]] <- prop[Block[[b]]] + rbind(U) %*% S[[b]]
        ### Log-Posterior
        Data[["prop"]] <- Block[[b]]
        ...
        if(inherits(Mo1, "try-error")) {
          ...
          {
            log.alpha <- 0 # setting minimum to 1 on natural scale
          }
          if(log.u < log.alpha) {
            Mo0 <- Mo1
            Data[["Gene.LL"]] <- Mo0$Parameter.LL
        ...
        ### Adaptation
        S[[b]] <- adapt_S(S[[b]],U,exp(log.alpha),iter,target=alpha.star,gamma=gamma)
        VarCov[[b]] <- S[[b]] %*% t(S[[b]])
...
.mcmcrwm_local <- function(Model, Data, Iterations, Status, Thinning, Specs,
  ...
  if(length(Block) == 0) {
    ...
    for (iter in 1:Iterations) {
      ...
      for (b in 1:B) {
        ### Propose new parameter values
        prop <- Mo0[["parm"]]
        prop[Block[[b]]] <- Mo0[["parm"]][Block[[b]]] + c(t(chol(VarCov[[b]])) %*% rnorm(length(Block[[b]])))
        Data[["prop"]] <- Block[[b]]
