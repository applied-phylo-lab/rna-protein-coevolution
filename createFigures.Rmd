...
div.p <- ggplot(dist.cor,aes(x=Distance/2,y=Correlation,color=Measure)) +
  geom_point() +
  theme_cowplot() +
...
### Check for consistency across runs - mostly resume code that can be run without the R objects

Here, I will be looking at the runs where the human species is dropped from our analysis. This first chunk is just to create summary flat files from the R objects output by LaplacesDemon. These commands have been commented out and summary files should be abvailable in the GitHub repo.
...
# createSummaryFile("Results/Drop_human/Essential_vs_non_essential/Essential")
# createSummaryFile("Results/Drop_human/Essential_vs_non_essential/Non_essential")
# createSummaryFile("Results/Drop_human/Haplosufficient_vs_insufficient/Haploinsufficient")
# createSummaryFile("Results/Drop_human/Haplosufficient_vs_insufficient/Haplosufficient")
...
The impact of gene expression on the evolutionary patterns observed across species of orthologous proteins is well-established. Here, we examine if there are systematic differences in parameters as a function of gene expression. Protein-driven models were fit to decile bins based on independent human protein abundance data from PaxDB (whole organism, integrated).
...
Below, we set up a data frame that contains data gene regulation and eQTL data vary across bins. We correlate our parameters with this data.
...
```
Below are violin and scatter plots showing the relationship of the different data with gene expression. Most of the relationships are weak. 
...
Aside from gene expression, gene essentiality is also expected to result in stronger selection and thus higher rates of adaptation compared to non-essential genes. Essential genes are based on a set of CRISPR knockouts in human cell lines and were obtained from the references noted in the manuscript and relevant README file. 

```{r}
toplevel <- list.files(path="Results/Drop_human/Essential_vs_non_essential/",pattern = "[A-Za-z_]+",include.dirs = T)
mcmc.rda <- list.files(path=file.path("Results/Drop_human/Essential_vs_non_essential",toplevel,"Protein_selection_strong","2024-11-01_prot_determines_mrna_prior_based_on_vertlife_mcc_node_dated_tree_random_start"),pattern="summary.tsv",recursive = T,full.names = T)

#ess.param.table <- createParamTableFromRobjects(mcmc.rda,pattern="(Essential|Non_essential)/[A-Za-z_]+")
ess.param.table <- createParamTableFromSummary(file.path.summary = mcmc.rda, pattern="(Essential|Non_essential)/[A-Za-z_]+")
```
...
### THIS R CODE CHUNK WILL REQUIRE THE R OBJECT FILES
...
We also assess if parameters vary based on if a gene is haploinsufficient or not. 


```{r}
toplevel <- list.files(path="Results/Drop_human/Haplosufficient_vs_insufficient/",pattern = "[A-Za-z_]+",include.dirs = T)
mcmc.rda <- list.files(path=file.path("Results/Drop_human/Haplosufficient_vs_insufficient/",toplevel,"Protein_selection_strong","2024-11-01_prot_determines_mrna_prior_based_on_vertlife_mcc_node_dated_tree_random_start"),pattern="summary.tsv",recursive = T,full.names = T)

#haplo.param.table <- createParamTable(mcmc.rda,pattern="(Haplosufficient|Haploinsufficient)/[A-Za-z_]+")
haplo.param.table <- createParamTableFromSummary(file.path.summary = mcmc.rda, pattern="(Haplosufficient|Haploinsufficient)/[A-Za-z_]+")
```
