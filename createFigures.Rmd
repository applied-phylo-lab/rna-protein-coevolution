...
tree.p
ggsave2("Figures/phylo_tree.pdf",tree.p,width=9)
...
pairwise.prot <- pairwise.prot[tree.no.human$tip.label,tree.no.human$tip.label]
pairwise.mrna <- pairwise.mrna[tree.no.human$tip.label,tree.no.human$tip.label]
pairwise.prot[upper.tri(pairwise.prot)] <- pairwise.mrna[upper.tri(pairwise.mrna)]

tree.dend <- as.dendrogram(tree.no.human)

diag(pairwise.prot) <- NA
prot.cor <- Heatmap(pairwise.prot,
        col = hcl.colors(25, palette = "Blues 3"),
        na_col = "black",
...
createParamTable <- function(filepaths.mcmc.obj,pattern="[0-9]{1,2}-[0-9]{2,3}/[A-Za-z_]+",adapt=F)
{
  names(filepaths.mcmc.obj) <- extractRunNameFromFilePath(mcmc.rda,pattern=pattern)
...
getParamTableForSetofRuns <- function(toplevel,mcmc.pattern,model.type="Protein_selection_strong",adapt=F)
{
...
prot.strong.mcmc.rda <- list.files(path=file.path("Results/Drop_human/Bin_based_on_paxdb_human_prot_abund/",toplevel,"Protein_selection_strong","2024-11-01_prot_determines_mrna_prior_based_on_vertlife_mcc_node_dated_tree_random_start"),pattern="rwm_mcmc_1.Rda",recursive = T,full.names = T)
prot.weak.mcmc.rda <- list.files(path=file.path("Results/Drop_human/Bin_based_on_paxdb_human_prot_abund/",toplevel,"Protein_selection_weak","2024-11-14_mrna_determines_prot_off_diagonal_sigma_prior_based_on_vertlife_mcc_node_dated_tree_random_start"),pattern="rwm_mcmc_1.Rda",recursive = T,full.names = T)
mcmc.rda <- c(prot.strong.mcmc.rda,prot.weak.mcmc.rda)



# toplevel <- list.files(path="Results/Drop_human/Bin_based_on_species_mean_protein/",pattern = "^[0-9]{1,2}-[0-9]{2,3}$",include.dirs = T)
# mcmc.rda <- list.files(path=file.path("Results/Drop_human/Bin_based_on_species_mean_protein/",toplevel,"Protein_selection_strong","2024-11-11_prot_determines_mrna_prior_based_on_vertlife_mcc_node_dated_tree_random_start"),pattern="rwm_mcmc_1.Rda",recursive = T,full.names = T)


# toplevel <- list.files(path="Results/Drop_human/Bin_based_on_paxdb_human_prot_abund_essential/",pattern = "^[0-9]{1,2}-[0-9]{2,3}$",include.dirs = T)
# mcmc.rda <- list.files(path=file.path("Results/Drop_human/Bin_based_on_paxdb_human_prot_abund_essential/",toplevel,"Protein_selection_strong","2024-11-01_prot_determines_mrna_prior_based_on_vertlife_mcc_node_dated_tree_random_start"),pattern="rwm_mcmc_1.Rda",recursive = T,full.names = T)
...
prot.dom.param.table <- createParamTable(mcmc.rda,adapt = adapt) %>%
...
number.enhancers.mouse <- read_csv("Data/CREr/EnhancerAtlas/mouse_kidney_enhancer_counts_per_gene_w_human_id.csv") %>% 
...
sel.vs.cre <- ggplot(param.table.w.enh.eqtl %>% filter(Parameter.type=="Selection"),aes(x=Median.Num.CREs.Human,y=Mean,color=Abundance)) +
  geom_point() +
...
tau.diff.vs.cre.human <- ggplot(tau.w.enh.eqtl,aes(x=Median.Num.CREs.Human,y=Sigma.Diff)) +
  geom_point() +
  xlab("Median number of tCREs per gene") +
  stat_cor(method="spearman",aes(label=..r.label..),show.legend = F) +
  ylab(expression("Log("*tau[R]*") - Log("*tau[P]*")")) +
