  ...
  lp <- sum(unlist(lapply(1:num.genes, function(current.loci)
    {
      ...
      tmp <- c(par[1],par[2],par[3],par[4],par[par.index],par[par.index+1],par[5],par[6],par[7])
      ll <- ll_fun[[current.loci]](tmp)
...
# Treat the first 7 parameters as the H and Sigma matrix. Remaining are Theta_A, Theta_B for each loci
# | Alpha A | Alpha BA | Alpha AB | Alpha B | Sigma A | Sigma AB | Sigma B |
log_post_hier_parallel_no_root <- function(par,ll_fun)
{
  lp <- 0
  hypr <- hyper_prior_bayou(par[1:7])
  num.genes <- (length(par)-7)/2
...
true.param <- c(start.alpha.a,
               ...
               start.sig.ab,
               start.sig.b)
names.true.param <- c("H_A","H_BA","H_AB","H_B","Sigma_A","Sigma_AB","Sigma.B")

for (i in 1:n)
{
  true.param <- c(true.param,x0.a[i],x0.b[i],theta.a[i],theta.b[i])
  names.true.param <- c(names.true.param,paste0(c("X0_A","X0_B","Theta_A","Theta_B"),"_",i))
}
names(true.param) <- names.true.param


true.post <- log_post_hier(true.param,ll_fun = likFun.list)
...
for (i in 1:n)
{
  ...
  modelOU <- PCM(model="OU__Omitted_X0__Schur_WithNonNegativeDiagonal_Transformable_H__Theta__UpperTriangularWithDiagonal_WithNonNegativeDiagonal_Sigma_x__Omitted_Sigmae_x",k=2)

  likFun.list[[i]] <- PCMCreateLikelihood(data.pcm[[i]], ba.tree, modelOU)
}
...
n.batch <- 10000
fit.hier.OU.no.root <- MCMC(log_post_hier_no_root, n.batch, init = true.param, scale = rep(1,length(true.param)),
            adapt = T, acc.rate = 0.25, gamma = 2/3, ll_fun=likFun.list)
